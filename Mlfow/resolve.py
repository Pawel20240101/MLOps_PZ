...
if __name__ == "__main__":
    ...
    if champ_mv and chall_mv:
        ...
        f1_champ = champ_run.data.metrics["f1_score"]

        chall_run = client.get_run(chall_mv.run_id)
        f1_chall = chall_run.data.metrics["f1_score"]

        if f1_chall >= f1_champ:
            logger.info("Challenger model surpassed current champion, promoting challenger to champion.")
            client.delete_registered_model_alias(MODEL_NAME, "challenger")
            client.set_registered_model_alias(MODEL_NAME, "champion", chall_mv.version)
        else:
            logger.error("Challenger did not surpass champion. Ending.")
            raise Exception("Challenger model does not surpass current champion.")
    elif champ_mv and chall_mv is None:
        logger.info("No challenger model found. Continuing with champion.")
